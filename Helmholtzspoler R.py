...
xe, Be = np.loadtxt('Målte verdier Helmholtzspoler a=R.txt', unpack=True, delimiter = ' ,')
...
plt.title('Helmholtzspole a=R')
plt.ylim([0, 1.1*np.max(Bb + deltaBb)])
plt.xlabel('$x$ [m]')
plt.ylabel('$B$ [Gauss]')
plt.legend(loc='upper left');
plt.savefig('Helmholtzspole_a=R_v3.pdf')
...
plt.title('Helmholtzspole_a=R relativ usikkerhet i prosent')
plt.ylim([-1.1*np.max(deltaBb_rev), 2.2*np.max(DB_rev_e)])
plt.xlabel('$x$ [m]')
plt.ylabel('Avvik [%]')
plt.legend(loc='upper center');
plt.savefig('Helmholtzspole_a=R_v3_relativusikkerhet.pdf')
